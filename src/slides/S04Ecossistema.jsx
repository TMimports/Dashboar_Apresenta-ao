const nodes = [
  { id: 'tm',    label: 'TM ELÉTRICAS',      sub: 'Importação + Atacado',    x: 50, y: 8,  color: '#FF6B00', size: 'lg' },
  { id: 'tecle', label: 'TECLE MOTOS',        sub: 'Varejo + Distribuição',   x: 50, y: 36, color: '#FFFFFF', size: 'md' },
  { id: 'at',    label: 'ASSISTÊNCIA TÉCNICA',sub: 'Pós-venda especializado', x: 50, y: 60, color: '#AEAEB2', size: 'sm' },
  { id: 'tb',    label: 'TECLEBOM',           sub: 'Proteção Veicular',       x: 50, y: 78, color: '#AEAEB2', size: 'sm' },
  { id: 'cf',    label: 'CLIENTE FINAL',      sub: 'Experiência completa',    x: 50, y: 94, color: '#8E8E93', size: 'xs' },
]
  ...
  return (
    <div className="relative w-full h-full flex flex-col px-16 py-10 overflow-hidden"
      ...
      <div className="mb-6 animate-fade-up">
        ...
        <h2 className="text-4xl font-black text-white">Um ecossistema integrado de mobilidade elétrica</h2>
        <p className="text-[#8E8E93] mt-1 text-sm">Da importação ao cliente final</p>
      </div>

      <div className="flex gap-12 flex-1 min-h-0">
        {/* Flow visual */}
        <div className="flex flex-col justify-between flex-1 py-4 animate-fade-in delay-200">
          {nodes.map((node, i) => (
            <div key={node.id} className="flex flex-col items-center">
              {i > 0 && (
                <div className="flex flex-col items-center mb-2">
                  <div className="w-0.5 h-4" style={{ background: 'linear-gradient(#2C2C2E, #3A3A3C)' }} />
                  <div className="w-2 h-2 rounded-full" style={{ background: '#3A3A3C' }} />
                  <div className="w-0.5 h-4" style={{ background: 'linear-gradient(#3A3A3C, #2C2C2E)' }} />
                </div>
              )}
              <div
                className="flex flex-col items-center text-center px-6 py-3 rounded-xl transition-all duration-300 cursor-default group
                  hover:scale-105"
                style={{
                ...
                <p className="font-black text-sm tracking-wide" style={{ color: node.color }}>{node.label}</p>
                <p className="text-xs mt-0.5" style={{ color: '#8E8E93' }}>{node.sub}</p>
              </div>
            </div>
          ))}
        </div>
        ...
        {/* Right panel */}
        <div className="w-80 flex flex-col gap-4 animate-slide-right delay-300">
          <div className="p-6 rounded-2xl flex-1" style={{ background: '#1C1C1E', border: '1px solid #2C2C2E' }}>
