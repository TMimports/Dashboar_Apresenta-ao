            ...
            <div key={i} className="flex items-stretch">
              <div className="flex flex-col items-center w-6 sm:w-8 mr-3 sm:mr-4 flex-shrink-0">
                <div className="w-2.5 h-2.5 rounded-full flex-shrink-0"
                  style={{ background: item.highlight ? '#FF6B00' : '#3A3A3C', marginTop: 14 }} />
                {i < chain.length - 1 && <div className="w-0.5 flex-1" style={{ background: 'linear-gradient(#3A3A3C, #2C2C2E)', minHeight: 6 }} />}
              </div>
              <div className="flex-1 flex items-center gap-3 px-3 sm:px-4 py-2.5 mb-1 rounded-xl transition-all duration-300 cursor-default group
                hover:border-[rgba(255,107,0,0.5)]"
                style={{ background: item.highlight ? 'rgba(255,107,0,0.06)' : '#1C1C1E', border: `1px solid ${item.highlight ? 'rgba(255,107,0,0.3)' : '#2C2C2E'}` }}>
        ...
        <div className="slide-sidebar flex flex-col gap-4 animate-slide-right delay-300">
          <div className="p-5 sm:p-6 rounded-2xl" style={{ background: '#1C1C1E', border: '1px solid #2C2C2E' }}>
            ...
            <p className="text-white text-sm leading-relaxed">
              A TM atua de <span style={{ color: '#FF6B00' }}>ponta a ponta</span> na cadeia do negócio, capturando margem em múltiplas etapas.
            </p>
          </div>
