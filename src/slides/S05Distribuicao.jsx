  ...
  return (
    <div className="relative w-full h-full flex flex-col px-16 py-10 overflow-hidden"
      ...
      <div className="mb-6 animate-fade-up">
        ...
        <h2 className="text-4xl font-black text-white">Distribuição já validada</h2>
      </div>
      ...
      <div className="flex gap-10 flex-1 min-h-0">
        ...
        <div className="flex flex-col gap-4 w-72 animate-slide-left delay-200">
          ...
          <div className="p-6 rounded-2xl text-center glow-orange"
            ...
            <p className="text-7xl font-black text-[#FF6B00] leading-none">22</p>
            <p className="text-white font-semibold mt-2">Pontos de Venda Ativos</p>
          ...
          <div className="flex flex-col gap-2">
            {channels.map((ch, i) => (
              <div key={i}
                className="flex items-center justify-between px-4 py-3 rounded-xl card-premium">
                <div className="flex items-center gap-3">
                  <div className="w-2.5 h-2.5 rounded-full flex-shrink-0" style={{ background: ch.color }} />
                  <span className="text-sm text-[#AEAEB2]">{ch.label}</span>
                </div>
                <span className="text-white font-bold text-lg">{ch.value}</span>
              </div>
            ))}
          </div>
        ...
        <div className="flex-1 flex flex-col animate-fade-in delay-300">
          <p className="text-xs text-[#8E8E93] uppercase tracking-widest font-medium mb-4">Distribuição por canal</p>
          <div className="flex-1" style={{ minHeight: 0 }}>
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={channels} margin={{ top: 10, right: 20, left: -20, bottom: 10 }}>
                <XAxis dataKey="label" tick={{ fill: '#8E8E93', fontSize: 12 }} axisLine={false} tickLine={false} />
